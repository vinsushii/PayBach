...
>>>>>>> ad0f60cc7d49f1e690e9a6a8c203ade70a5afa52
  ...
  if (images.length > 0) {
    imageList = images.map(path => "/PayBach/uploads/" + path.split("/uploads/")[1]);
  ...
  } else mainImage.src = "/PayBach/uploads/default.png";
  ...
  if (thumbs.length) {
    document.querySelector(".arrow.left").onclick = () => updateImage((index - 1 + thumbs.length) % thumbs.length);
    document.querySelector(".arrow.right").onclick = () => updateImage((index + 1) % thumbs.length);
    thumbs.forEach((t, i) => t.addEventListener("click", () => updateImage(i)));
  }
  ...
  seller.innerHTML = `👤 ${listing.first_name} ${listing.last_name}<br>📧 ${listing.email}`;
